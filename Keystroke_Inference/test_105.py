...
model.eval()
tap_point_folder = "data/105"
dataset_name_list = os.listdir(tap_point_folder)
tgt_folder = 'results'
model_dict=torch.load('inference_model.pth')
model.load_state_dict(model_dict)
for m in range(len(dataset_name_list)):
    dataset_name = dataset_name_list[m]
    dataset_folder = os.path.join(tap_point_folder,dataset_name)
    sub_folder_name_list = os.listdir(dataset_folder)
    sub_folder_name_list = [int(i) for i in sub_folder_name_list]
    sub_folder_name_list.sort()
    sub_folder_name_list = [str(i) for i in sub_folder_name_list]#
    cer_length_list = []
    ...
    np.savetxt(os.path.join(tgt_folder,dataset_name+".txt"),cer_length_list)
